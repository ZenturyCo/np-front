...
import { FolderKanban, CheckSquare, Video, Activity, AlertTriangle, Clock, TrendingUp, AlertCircle, RefreshCw } from "lucide-react";
...
const getDerivedProjectProgress = (
  projectId: string,
...
const { data: tasks = [], isLoading: loadingTasks, isError: errorTasks, refetch: refetchTasks } = useQuery({
  ...
  const activeTasks = tasks.filter((t) => t.status === "in_progress" || t.status === "todo" || t.status === "review");
  const completedToday = tasks.filter((t) => t.status === "done").length;
  const healthScore =
    projects.length > 0
      ? Math.round(
          projects.reduce((sum, project) => sum + getDerivedProjectProgress(project.id, tasks, project.progress), 0) /
            projects.length,
        )
      : 0;
  const activeProjectsCount = projects.filter((project) => project.rawStatus !== "completed" && project.rawStatus !== "cancelled").length;

  const metrics = [
    { title: "Projectos Activos", value: activeProjectsCount, icon: FolderKanban, trend: `${projects.length} no total`, trendUp: true, color: "primary" as const },
            ...
            <div className="space-y-4">
              {projects.slice(0, 5).map((p) => (
                <div key={p.id} className="space-y-2 rounded-lg bg-muted/50 p-3">
                  {(() => {
                    const progressValue = getDerivedProjectProgress(p.id, tasks, p.progress);
                    return (
                      <>
